...
r3b_ascii_paged_stack::r3b_ascii_paged_stack():
	...
	_back_buf( new std_deque<r3b_ascii_event> ),
	_memory_sz( 0 )
{
	pthread_attr_init( &_op_attribute );
	pthread_attr_setdetachstate( &_op_attribute, PTHREAD_CREATE_JOINABLE );
...
r3b_ascii_paged_stack::~r3b_ascii_paged_stack(){
	...
	while( !_pages.empty() ){ fclose( _pages.back() ); _pages.pop(); }
	
	//cleanup the attributes
	pthread_attr_destroy( &_op_attr );
	pthread_mutexattr_destroy( &__memory_sz_mutexattr );
}
...
//------------------------------------------------------------------------------------
//alloc and free for the tmp_file structure.
tmp_file *r3b_ascii_paged_stack::tmp_file_alloc( FILE *file, std::deque<r3b_ascii_event> *bb ){
	tmp_file *f = (tmp_file*)malloc( sizeof(tmp_file) );
	f->file = file;
	f->bb = bb;
...
void r3b_ascii_paged_stack::tmp_file_free( tmp_file *a_file ){ free( a_file ) };
...
void r3b_ascii_paged_stack::swap_buffers(){
	...
	if( _front_buf->empty() && !_back_buf->empty() ){
		...
		a_file = tmp_file_alloc( _pages.front(), _back_buf ); 
	...
	tmp_file_free( a_file ); //a thing dies where it's born.
}
...
void *r3b_ascii_paged_stack::page_out( void *a_file ){
	...
	tmp_file the_file = *(tmp_file*)a_file; //copy, safer
	...
	void *buf;
	while( !the_file.bb->empty() ){
		//retrieve the event and calc the buffer size.
		current_evt &the_file.bb->front();
		...
		//save it
		fwrite( buf, buf_size, 1, the_file->file );
		
		//and now, pop it and cleanup
		the_file.bb->pop_front();
		free( buf );
		
		//update the memory size
		pthread_mutex_lock( &_memory_sz_mutex );
		_memory_sz -= sizeof(r3b_ascii_event) +
