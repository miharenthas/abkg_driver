...
A function that quickly loads ASCII events meant to be used with the R3B ASCII generator and have been compressed with bzip2.\n\
...
DEFUN_DLD( r3bascii_slurp_compressed_events, args, , O_DOC_STRING ){
	...
	if( nargin == 1 && args(0).is_string() ){
		...
		strcat( command, args(0).string_value().c_str() );
		infile[0] = popen( command, "r" );
		...
		for( int i=0; i < nargin; ++i ){
			strcpy( command, "bunzip2 -c " );
			strcat( command, args(i).string_value().c_str() );
			infile[i] = popen( command, "r" );
	...
	for( int i=0; i < nargin; ++i ) pclose( infile[i] );
